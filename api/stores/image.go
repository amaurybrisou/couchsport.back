...
func (app ImageStore) Delete(userID uint, body io.Reader) (bool, error) {
	...
	if err := app.Db.Exec("UPDATE images AS i INNER JOIN pages AS p ON i.page_id = p.id SET i.deleted_at = NOW() WHERE i.id = ? AND p.owner_id = ?", image.ID, userID).Error; err != nil {
		log.Error(err)
		return false, err
	}
